...
**XR MetaData**는 Meta Quest 환경에서 구동되는 몰입형 WebXR 클라이언트와 웹 기반 관리자 인터페이스(Admin UI)를 연동하여, 3D 공간 내 오브젝트별 사운드(MP3) 및 진동 파형(.haptic)의 재생 구간(`segmentStart` ~ `segmentEnd`)을 매핑하고 실시간으로 검증할 수 있는 통합 저작 파이프라인을 제공합니다.
