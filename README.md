# WebXR 햅틱·오디오 메타데이터 저작 도구 (XR MetaData)
...
2. **WebXR 컨트롤러 햅틱 신호 캔슬 (Signal Cancellation)**:
...
3. **고폴리곤 3D 에셋의 실시간 VR 인터랙션 병목 (Raycast Overhead)**:
...
## 4. 핵심 트러블슈팅 및 기술적 해결 (Deep Troubleshooting)
...
* **해결 방법**:
  - 1ms 고주파 진동 파형을 컨트롤러 액추에이터의 최적 구동 주기인 **60ms 단위 윈도우 청크**로 실시간 리샘플링 및 분할(Slicing).
  - 해당 60ms 윈도우 구간의 RMS(Root Mean Square) 유효 진폭을 산출하여 단일 펄스로 정규화 스케줄링함으로써, 하드웨어 버퍼 과부하 없이 원본 파형의 질감을 살린 매끄러운 햅틱 피드백을 구현.

### 2) 히트박스 프록시(Hitbox Proxy)를 통한 고폴리곤 모델 90fps 방어
* **문제 현상**:
...
* **해결 방법**:
  ...
  - 고폴리곤 메시 자체의 레이캐스팅을 비활성화하고, 모델 외곽에 정확히 일치하는 비가시(Invisible) 전용 직육면체 히트박스(`BoxGeometry`)를 프록시로 생성.
...
## 7. 외부 에셋 라이선스 (Third-Party Credits)
